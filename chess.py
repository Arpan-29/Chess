...
class ChessBoard :
    ...
    def show(self) :
        ...
        for i in range(8) :
            ...
            # win.blit(text, (690, (8 - i) * self.width - 18))

        for j in range(8) :
            text = font.render(chr(65 + j), True, WHITE)
            # win.blit(text, ((j + 1) * self.width - 8, 10))
...
while run :
    ...
    if chess.in_check(chess.white_king_pos) :
        ...
        if total == 0 :
            ...
            text = font.render('CHECKMATE! BLACK WINS.', True, CYAN)
    ...
    if chess.in_check(chess.black_king_pos) :
        ...
        if total == 0 :
            font = pygame.font.SysFont('Consolas', 30)
            text = font.render('CHECKMATE! WHITE WINS.', True, WHITE)
            win.blit(text, (200, 5))
    ...
    if touched_piece :
        if 0 <= i < 8 and 0 <= j < 8 :
            ...
            for pos in options :
                ...
                # pygame.draw.rect(win, GRAY, (new_j * chess.width + 40, new_i * chess.width + 40, chess.width, chess.width), 5)
                # if chess.board[new_i][new_j].name is not None :
                #     win.blit(chess.board[new_i][new_j].img, (j * chess.width + 43, i * chess.width + 43))
